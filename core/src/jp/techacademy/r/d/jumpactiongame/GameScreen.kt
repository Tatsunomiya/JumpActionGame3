...
class GameScreen(private val mGame: MyGdxGame) : ScreenAdapter() , Sound {
    ...
    init{
        ...
        mBg.setSize(CAMERA_WIDHT,CAMERA_HEIGHT)

        mBg.setPosition(0f,0f)

        mCamera = OrthographicCamera()
        mCamera.setToOrtho(false,CAMERA_WIDHT,CAMERA_HEIGHT)
        mViewPort = FitViewport(CAMERA_WIDHT,CAMERA_HEIGHT,mCamera)
    ...
    override fun render(delta: Float) {
        ...
        mGame.batch.begin()
        mBg.setPosition(mCamera.position.x - CAMERA_WIDHT / 2 , mCamera.position.y - CAMERA_HEIGHT /2)
        ...
        mGame.batch.begin()
        mFont.draw(mGame.batch, "HighScore:  &HighScore", 16f, GUI_HEIGHT -15)
